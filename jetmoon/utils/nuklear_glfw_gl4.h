...
extern float nuklear_viewport[4];
extern int nuklear_changedViewport;
...
nk_glfw3_mouse_button_callback(GLFWwindow* window, int button, int action, int mods)
{
	...
	x = (x - nuklear_viewport[0]) * nuklear_viewport[2];
	y = (y - nuklear_viewport[1]) * nuklear_viewport[3];
...
nk_glfw3_new_frame(void)
{
	...
	if(nuklear_changedViewport == 0){
		nuklear_viewport[2] = glfw.width / viewport[2];
		nuklear_viewport[3] = glfw.height / viewport[3];
	}
	glfw.width = viewport[2];
	glfw.height = viewport[3];
	glfw.display_width = viewport[2];
	glfw.display_height = viewport[3];
